...
<template>
    <div>
        <div class="flex gap-3">
            <button class="btn btn-primary btn-sm" @click="resetMusic">Đặt lại danh sách nhạc</button>
            <label for="explore">
                <input type="file" class="" id="explore" style="display: none" @change="handleFileChange"
                    :accept="limitType" />
                <span class="btn btn-primary btn-sm">Tải nhạc lên</span>
            </label>
        ...
        <div>
            <ul class="p-0">
                <li v-for="item in localMusicListValue" :key="item.id" class="flex items-center gap-6 pb-2 mb-3 divide-y">
                    ...
                    <div class="flex gap-3">
                        <button class="btn btn-primary btn-xs" @click="play(item)">Play</button>
                        <button class="btn btn-error btn-xs" @click="deleteMusic(item)">Xóa</button>
                    </div>
                </li>
            </ul>
        </div>
